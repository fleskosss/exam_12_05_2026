...
app.config['SQLALCHEMY_DATABASE_URI'] = \
    'sqlite:///' + os.path.join(BASE_DIR, 'task4.db')
...
class User(db.Model, UserMixin):
    ...
    reviews = db.relationship('Review', backref='author', lazy=True) # Отличие от Task_3
...
class Request(db.Model):
    ...
    reviews = db.relationship('Review', backref='request', lazy=True) # Отличие от Task_3
...
def create_request(course_id):
    ...
    return render_template('create_request.html', course=course)


# Отличие от Task_3
@app.route('/request/<int:request_id>/review', methods=['POST'])
@login_required
def create_review(request_id):
    ...
    return redirect(
        url_for('course_page', course_id=request_item.course_id)
    )
